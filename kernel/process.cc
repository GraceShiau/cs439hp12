...
long Process::execv(const char* fileName, SimpleQueue<const char*> *args, long argc, bool checkPermissions) {
    ...
    char** argsStart = (char**)0xf0000000;
    char* addr = (char*)0xf0000000 + argc * 4;
    ...
    for(int a = 0; a < argc; ++a)
    {
    	...
    	const char* nextArgument = args->removeHead();
		justInCaseQueue->addTail(nextArgument);
    	const int argumentLength = K::strlen(nextArgument);
    	memcpy(addr, nextArgument, argumentLength);
    	addr[argumentLength] = 0;
    	addr+= argumentLength + 1;
    }

    long* stackPointer = (long*) userESP;
    stackPointer[0] = argc;
    stackPointer[1] = 0xf0000000;
    /* clear resources */
    resources->closeAll();
